...
from torch_geometric.nn import GCNConv, SAGPooling, JumpingKnowledge, global_mean_pool, global_max_pool
...
class GCN_SAGPool(torch.nn.Module):
    def __init__(self, in_dim, out_dim, ratio):
        ...
        self.pool = SAGPooling(out_dim, ratio=ratio)
...
class SAGPool(torch.nn.Module):
    def __init__(self, in_dim, hidden_dim, num_class, num_layer=3, ratio=.5):
        ...
        for i in range(num_layer):
            if i == 0:
                self.conv_pool_layers.add_module(f'conv_pool{i}', GCN_SAGPool(in_dim, hidden_dim, ratio))
            else:
                self.conv_pool_layers.add_module(f'conv_pool{i}', GCN_SAGPool(hidden_dim, hidden_dim, ratio))
