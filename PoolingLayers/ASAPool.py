...
from torch_geometric.nn import GCNConv, ASAPooling, JumpingKnowledge, global_mean_pool, global_max_pool
...
class GCN_ASAPool(torch.nn.Module):
    def __init__(self, in_dim, out_dim, ratio):
        ...
        self.pool = ASAPooling(out_dim, ratio=ratio)
...
class ASAPool(torch.nn.Module):
    def __init__(self, in_dim, hidden_dim, num_class, num_layer=3, ratio=.5):
        ...
        for i in range(num_layer):
            if i == 0:
                self.conv_pool_layers.add_module(f'conv_pool{i}', GCN_ASAPool(in_dim, hidden_dim, ratio))
            else:
                self.conv_pool_layers.add_module(f'conv_pool{i}', GCN_ASAPool(hidden_dim, hidden_dim, ratio))
...
for i in range(num_experiment):
    model = ASAPool(dataset.num_features, hidden_dim, dataset.num_classes, 3).to(device)
